...
lib <- c("nnet","here","dplyr","ggplot2","GGally","dplyr","ggcorrplot","car","smotefamily","skimr","Metrics","patchwork","ggplot")

lapply(lib, library, character.only = TRUE)
opossum <- read.csv(here("data/processed", "possum.csv"))
dataset<- opossum
...
print(variabili_con_null)

#1)Can we use total length to predict a possum's head length?
cor(hdlngth,totlngth)
a<-lm(hdlngth ~ totlngth, data=dataset)
summary(a)
ggplot(dataset, aes(x = totlngth, y = hdlngth)) +
  geom_point(color = "black") +  # Punti arancioni
  geom_smooth(method = "lm", se = FALSE, color = "orange") +  # Linea di regressione nera
  labs(x = "Tot length", y = "Head length") +  # Etichette degli assi
  theme_bw()


#2)Which possum body dimensions are most correlated with age and sex?
dataset <- dataset %>%
  mutate(sex = ifelse(sex == "m", 0, ifelse(sex == "f", 1, NA)))
dataset <- dataset %>%
  mutate(Pop = ifelse(Pop == "Vic", 1, ifelse(Pop == "other", 0, NA)))

correlation_matrix <- cor(dataset) #cration of the matrix
ggcorrplot(correlation_matrix, type = "full", lab = TRUE)


#3) Can we classify a possum's sex by its body dimensions and location?

# Creazione del modello iniziale con tutte le variabili
initial_model <- glm(sex ~ ., data = dataset, family = binomial)

# Esegui la selezione stepwise
final_model <- step(initial_model)

b<-glm(sex ~ site + hdlngth + totlngth + earconch + eye , data=dataset, family = binomial)
summary(b)

#4) Can we predict a possum's trapping location from its body dimensions?

# Creazione del modello iniziale con tutte le variabili
initial_model <- lm(site ~ ., data = dataset )

# Esegui la selezione stepwise
final_model <- step(initial_model,  direction = "both")

b<-multinom(site ~ age + skullw + totlngth + taill + earconch + eye + chest + 
              belly , data=dataset)
summary(b)


# Creazione del modello iniziale con tutte le variabili
initial_model <- multinom(site ~ ., data = dataset )
...
all_sites <- unique(dataset$site)

ggplot(dataset, aes(x = site)) +
  geom_bar(fill = "orange", color = "black") +
  labs(title = "Barplot for 'Site'", x = "Site", y = "Frequency") +
  scale_x_continuous("Sites", labels = as.character(all_sites), breaks = all_sites)+
  theme_bw()


#Delivery
"""
For information about the data, see:
...
"""
variabili <- c("label","GENDER", "Car_Owner", "Propert_Owner", "CHILDREN", "Annual_income",
...
variabili <- c("label","Car_Owner", "Propert_Owner", "CHILDREN", "Annual_income","Birthday_count",  "Work_Phone", "Phone", "EMAIL_ID", "Family_Members",
...
lm_fit<- glm(label ~ ., data = prova, family = binomial(link = "logit"))
...
variabili_1<-c("label","Type_Income_mean", "GENDER_mean", "EDUCATION_mean", "Type_Occupation_mean", "Marital_status_mean", "Housing_type_mean","Employed_days")
...
lm.fit <- glm(label ~ Type_Income_mean + Employed_days, data = dataset)
vif(lm.fit)
...
variabili_1<-c("label","Type_Income_mean", "GENDER_mean", "EDUCATION_mean", "Type_Occupation_mean", "Marital_status_mean", "Housing_type_mean")
...
lm_fit <- glm(label ~ ., data = dtrain, family = binomial(link = "logit"))
